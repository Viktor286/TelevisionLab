  ...
  <input type="hidden" name="video" value="<? echo $OutId; ?>">
    <table>
      ...
      <tr>
        ...
        <a class="bt_download" href="http://sfrom.net/https://vimeo.com/<? echo $OutId; ?>" target="_blank"></a>
		<? 
...
if ($VimeoEmbed_flag == 'New') {echo '<div class="VideoBlock">'.$NewVimeoEmbed.'</div>';} else {echo '<div class="VideoBlock">'.$OldVimeoEmbed.'</div>';}
...
<div id="tabs-container">
    <ul class="tabs-menu">
        <li class="current"><a href="#tab-1"><img src="img/btn_a_sphere.png"></a></li>
        <li><a href="#tab-2"><img src="img/btn_a_fashion.png"></a></li>
        <li><a href="#tab-3"><img src="img/btn_a_art.png"></a></li>
        <li><a href="#tab-4"><img src="img/btn_a_music.png"></a></li>
        <li><a href="#tab-5"><img src="img/btn_a_other.png"></a></li>
    </ul>
    <div class="tab">
        <div id="tab-1" class="tab-content">
        <div class="tabDescr">
        <h2>Сфера деятельности</h2>
        <p>Ключевые слова, обозначающие сферы человеческой деятельности, связанные с роликом.</p>
        <textarea id="sa" class="tags" name="sa" placeholder="Scial acivity sphere tags..."/><? if (isset($getTags_SA) and strlen($getTags_SA) > 2) {echo $getTags_SA;} else {echo $Tags_SA;};?></textarea>
        ...
        <h2>Мода</h2>
        <p>Ключевые слова, обозначающие сферы человеческой деятельности, связанные с роликом.</p>
        <textarea id="fashion" class="tags" name="fashion" placeholder="Fashion style related tags..."/><? if (isset($getTags_Fashion)) {echo $getTags_Fashion;} else {echo $Tags_Fashion;};?></textarea>
        ...
        <textarea id="arts" class="tags" name="arts" placeholder="Fine arts related tags..."/><? if (isset($getTags_Arts) and strlen($getTags_Arts) > 2) {echo $getTags_Arts;} else {echo $Tags_Arts;};?></textarea>
        ...
        <h2>Музыка</h2>
        <p>Ключевые слова, обозначающие сферы человеческой деятельности, связанные с роликом.</p>
        <textarea id="music" class="tags" name="music" placeholder="Music genre related tags..."/><? if (isset($getTags_Music) and strlen($getTags_Music) > 2) {echo $getTags_Music;} else {echo $Tags_Music;};?>
</textarea>
        ...
        <h2>Другое</h2>
        <p>Ключевые слова, обозначающие сферы человеческой деятельности, связанные с роликом.</p>
        <textarea id="tags" class="tags" name="tags" placeholder="Other useful tags..."/><? if (isset($getTags_Others) and strlen($getTags_Others) > 2) {echo $getTags_Others;} else {echo $Tags_Others;};?></textarea>
...
<br style="clear:both;" />
        ...
        <td width="650" data-title="primary" class="primary">
                ...
                <? if (isset($_SESSION['user_name'])) {
				echo '<a class="bt_help" href="img/collection_classes.png"></a>';
			}?>
        
        
                  <div class="CanvasLine<? if (!Check_Title($getTitle) and empty($Title)) {echo '  input_error';} ?>"> 
            <!--<div class="Header"></div>-->
            <div class="Content"><? echo $nme_Title ?> [<a class="delete" href="javascript:void(0)">X</a>] <span class="realy_delete" style="display:none;">
          ...
          <div class="CanvasLine<? if (!Check_Title($getAuthor)and empty($Authors)) {echo '  input_error';} ?>"> 
            <!--<div class="Header"></div>-->
            <div class="Content_Inline"><? echo $nme_Author ?>
              <input type="text" class="medium" name="authors" value="<? if (!isset ($getAuthors)) {echo $Authors;} else {echo $getAuthors;} ?>">
            </div>
            <div class="Content_Inline"><? echo $nme_Location ?>
              <input type="text" class="short" name="location" value="<? if (isset ($getLocation)) {echo $getLocation;} else {echo $Location;} ?>">
            </div>
          </div>
          <div class="CanvasLine<? if (!Check_Title($getYear)and empty($Year)) {echo '  input_error';} ?>"> 
            ...
            <div class="Content_Inline<? if (!Check_Title($getBrand) and empty($Brand)) {echo '  input_error';} ?>"><? echo $nme_Brand ?>
            ...
            <div class="Content_Inline"><? echo $nme_Tv_Channel ?>
              <input type="text" class="short" name="tv" value="<? if (!isset ($getTv_Channel)) {echo $Tv_Channel;} else {echo $getTv_Channel;} ?>"> 
            </div>
          </div>
        ...
        <td data-title="flags" class="flags"><div class="CanvasLine">
            ...
            <div class="Content">
                ...
                <select name="broadcast" id="broadcast">
                    <option value="0" data-imagesrc="img/bt_type_id.png" data-description="<?= $nme_cat_Identity_cap ?>" <? isSelected ($getBroadcast_Type,0); ?>><?= $nme_cat_Identity ?></option>
                    <option value="1" data-imagesrc="img/bt_type_adv.png" data-description="<?= $nme_cat_Advertising_cap ?>" <? isSelected ($getBroadcast_Type,1); ?>><?= $nme_cat_Advertising ?></option>
                    <option value="2" data-imagesrc="img/bt_type_pr.png" data-description="<?= $nme_cat_Presentation_cap ?>" <? isSelected ($getBroadcast_Type,2); ?>><?= $nme_cat_Presentation ?></option>
                    <option value="3" data-imagesrc="img/bt_type_information.png" data-description="<?= $nme_cat_Information_cap ?>" <? isSelected ($getBroadcast_Type,3); ?>><?= $nme_cat_Information ?></option>
                    <option value="4" data-imagesrc="img/bt_type_entertainment.png" data-description="<?= $nme_cat_Entertainment_cap ?>" <? isSelected ($getBroadcast_Type,4); ?>><?= $nme_cat_Entertainment ?></option>
                    <option value="5" data-imagesrc="img/bt_type_art.png" data-description="<?= $nme_cat_Artistic_cap ?>" <? isSelected ($getBroadcast_Type,5); ?>><?= $nme_cat_Artistic ?></option>
                    <option value="6" data-imagesrc="img/bt_type_education.png" data-description="<?= $nme_cat_Educational_cap ?>" <? isSelected ($getBroadcast_Type,6); ?>><?= $nme_cat_Educational ?></option>
                </select>
